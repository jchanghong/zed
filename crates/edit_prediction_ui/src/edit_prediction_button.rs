...
impl EditPredictionButton {
                    ...
                    menu = menu.item(
                        entry
                            ...
                            .documentation_aside(DocumentationSide::Left, move |_cx| {
                                Label::new(format!("无法为此缓冲区切换编辑预测,因为已为 {} 禁用", language.name()))
                                    .into_any_element()
                            })
                    );
                }
        ...
        menu = menu
                ...
                .item(
                    ContextMenuEntry::new("Eager")
                        ...
                        .documentation_aside(DocumentationSide::Left, move |_| {
                            Label::new("当无语言服务器补全时,内联显示预测").into_any_element()
                        })
                        .handler({
                            let fs = fs.clone();
                            move |_, cx| {
                ...
                .item(
                    ContextMenuEntry::new("Subtle")
                        ...
                        .documentation_aside(DocumentationSide::Left, move |_| {
                            Label::new("仅在按住修饰键时内联显示预测(默认为 Alt)").into_any_element()
                        })
                        .handler({
                            let fs = fs.clone();
                            move |_, cx| {
    ...
    fn build_copilot_context_menu(
        ...
        ContextMenu::build(window, cx, |menu, window, cx| {
            ...
            menu.separator()
                .item(
                    ContextMenuEntry::new("Copilot: 下一步编辑建议")
            ...
            if needs_sign_in {
                    ...
                    .and_then(|provider| provider.usage(cx))
                {
                    ...
                    menu = menu
                        .custom_entry(
                            |_window, _cx| {
                                Label::new("您的 GitHub 账户创建时间不足 30 天")
                                    .size(LabelSize::Small)
                                    .color(Color::Warning)
                                    .into_any_element()
                            },
