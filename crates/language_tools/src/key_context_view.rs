...
impl Render for KeyContextView {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl ui::IntoElement {
        ...
        v_flex()
            ...
            .child(Label::new("这个视图可帮助你确定在 Zed 中创建自定义键位绑定时的当前上下文栈。触发键盘快捷键时，它还会显示所有可能触发的上下文，以及最终匹配的是哪一个。"))
            .child(
                h_flex()
                    .mt_4()
                    .gap_4()
            ...
            .child(Label::new("最近一次按键").mt_4().size(LabelSize::Large))
            ...
            .when_some(key_equivalents, |el, key_equivalents| {
                el.child(Label::new("按键等价项").mt_4().size(LabelSize::Large))
                    .child(Label::new("使用某些字符定义的快捷键已被重新映射，因此无需按住 option 键也能输入这些快捷键。"))
                    .children(
                        key_equivalents
                            .iter()
                            .sorted()
