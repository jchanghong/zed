                ...
                .menu(move |window, cx| {
                    ...
                    let menu = ContextMenu::build(window, cx, move |menu, _, _| {
                        menu.context(focus.clone())
                            ...
                            .action("扩展选区", Box::new(SelectLargerSyntaxNode))
                            ...
                            .separator()
                            .action_disabled_when(!has_diff_hunks, "下一个变更块", Box::new(GoToHunk))
                ...
                .menu(move |window, cx| {
                    let menu = ContextMenu::build(window, cx, {
                        ...
                        |mut menu, _, _| {
                            ...
                            if supports_minimap {
                                menu = menu.toggleable_entry("缩略图", minimap_enabled, IconPosition::Start, Some(editor::actions::ToggleMinimap.boxed_clone()), {
                                    let editor = editor.clone();
                                    move |window, cx| {
                                        editor
                                            .update(cx, |editor, cx| {
                            ...
                            if has_edit_prediction_provider {
                                ...
                                if !edit_predictions_enabled_at_cursor {
                                    edit_prediction_entry = edit_prediction_entry.documentation_aside(DocumentationSide::Left, |_| {
                                        Label::new("无法为此文件切换编辑预测，因为它位于排除文件列表中。").into_any_element()
                                    });
                                }
                            ...
                            if is_full {
                                ...
                                if supports_inline_diagnostics {
                                    ...
                                    if !diagnostics_enabled {
                                        inline_diagnostics_item = inline_diagnostics_item.disabled(true).documentation_aside(DocumentationSide::Left, |_|  Label::new("启用常规诊断之前，行内诊断不可用。").into_any_element());
                                    }
